...
switch ($_REQUEST['command']) {
    ...
    ");
        ...
        try {
            // Check total available containers
            $remainingContainers = 0;
            $result = $conn->query("SELECT SUM(containers_remaining) AS total_remaining FROM containers");
            if ($row = $result->fetch_assoc()) {
                $remainingContainers = (int)$row["total_remaining"];
            }
            if ($remainingContainers < $container) {
                throw new Exception("Insufficient remaining containers.");
            }
        ...
        ");
            ...
            //Deduct containers & calculate profit
            $containers_needed = $container;
            $result2 = $conn->query("SELECT id, containers_remaining, unit_price FROM containers WHERE containers_remaining > 0 ORDER BY id ASC LIMIT 2");
           
            // Fetch single container ID based on item_id
            $container_id = null;
            $stmtCont = $conn->prepare("SELECT id FROM containers WHERE item_id = ? LIMIT 1");
            $stmtCont->bind_param("i", $item_id);
            $stmtCont->execute();
            $stmtCont->bind_result($container_id);
            $stmtCont->fetch();
            $stmtCont->close();

            if ($container_id === null) {
                throw new Exception("No container found for the given item_id.");
            }
            ...
            while ($containers_needed > 0 && ($row2 = $result2->fetch_assoc())) {
                ...
                if ($available >= $containers_needed) {
                    ...
                    $stmtProfit = $conn->prepare("INSERT INTO transactioncontainerprofits 
                        ...
                        VALUES (?, ?, ?, ?, NOW())");
                    $stmtProfit->bind_param("iidi", $transaction_id, $container_id, $profit, $containers_needed);
                    $stmtProfit->execute();
                    $stmtProfit->close();
                    ...
                    // take what's left and continue to next batch
                    $profit = ($sold_price - $cost_price) * $available;

                    $stmtProfit = $conn->prepare("INSERT INTO transactioncontainerprofits 
                        (transactions_id, containers_id, profit, no_container, date_created) 
                        VALUES (?, ?, ?, ?, NOW())");
                    $stmtProfit->bind_param("iidi", $transaction_id, $container_id, $profit, $available);
                    $stmtProfit->execute();
                    $stmtProfit->close();

                    $conn->query("UPDATE containers 
                                SET containers_remaining = 0 
                                WHERE id = " . (int)$row2['id']);

                    $containers_needed -= $available;
                }
            }
            // while ($row2 = $result2->fetch_assoc() and $containers_needed > 0) {
            //     $cid = (int)$row2["id"];
            //     $available = (int)$row2["containers_remaining"];
            //     $cost_price = (float)$row2["unit_price"]; // from batch
            ...
            //         $conn->query("INSERT INTO transactioncontainerprofits (transactions_id, containers_id, profit, no_container, date_created)
            //                 VALUES ($transaction_id, $cid, $profit, $containers_needed, NOW())");

            //         $conn->query("UPDATE containers SET containers_remaining = containers_remaining - $containers_needed WHERE id = $cid");
            ...
            //         $conn->query("INSERT INTO transactioncontainerprofits (transactions_id, containers_id, profit, no_container, date_created)
            //                 VALUES ($transaction_id, $cid, $profit, $available, NOW())");

            //         $conn->query("UPDATE containers SET containers_remaining = 0 WHERE id = $cid");
    ...
    case "fetch_items":
        $stmt = $conn->prepare("SELECT i.id, i.item_name, c.unit_price FROM item i JOIN containers c ON c.item_id = i.id ORDER BY i.item_name ASC");
        $stmt->execute();
