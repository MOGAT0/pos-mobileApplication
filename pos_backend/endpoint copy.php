...
switch ($_REQUEST['command']) {
    ...
    case "search_customers":
        ...
        $sql = "SELECT DISTINCT custname FROM transactions WHERE custname LIKE '%$query%' LIMIT 10";
        $result = mysqli_query($conn, $sql);

        $customers = [];
        if ($result) {
            while ($row = mysqli_fetch_assoc($result)) {
                $customers[] = $row["custname"];
            }
        }
    ...
    case "fetch_transactions":
        // Search and area are optional
        $search = isset($response_json["search"]) ? anti_sql($response_json["search"]) : "";
        $area = isset($response_json["area"]) ? anti_sql($response_json["area"]) : "";

        // If both empty, return empty array (no need to show anything)
        if (empty($search) && empty($area)) {
            print_Jsonresponse(["transactions" => []]);
        }

        $sql = "SELECT transactID, custname, area, item_name, item_weight, total_amount 
                FROM transactions WHERE 1";

        if (!empty($search)) {
            $sql .= " AND custname LIKE '%$search%'";
        }
